...
@attr.s
class SpringContractTestExporter:
    name: str = "Spring Contract Test (Java)"
    output_ext: str = "groovy"

    def export_data(self, api_calls: list[ApiCall]):
        test_file_header = """
package contracts.httprider
...
"""
        ...
        return highlight(test_file_header, JavaLexer(), HtmlFormatter()) + "<br/>".join(output)
...
Contract.make {{
    description(\"\"\"
    Represents a successful scenario of posting a greeting
    ```
    given:
        a valid greeting POST request
    when:
...
exporter = SpringContractTestExporter()
