...
class AssertionBuilderPresenter:
    ...
    def __init__(self, parent_view):
        ...
        self.assertion_list_presenter = AssertionListPresenter(
            self, self.parent_view.tbl_assertions, self.parent_view
        )
    ...
    def on_request_body_selected(self, json_path, current_value):
        selector, _ = functools.reduce(
            self.json_path_builder, json_path[1:], ("$", dict)
        )
    ...
    def load_configuration_dialog(self, api_call: ApiCall):
        ...
        self.current_api_test_case = app_settings.app_data_cache.get_api_test_case(
            self.current.id
        )
    ...
    def on_close(self):
        self.current_api_test_case.assertions = (
            self.assertion_list_presenter.get_all_assertions()
        )
