...
from . import HeadersAssertionPresenter, BodyAssertionPresenter, AssertionListPresenter, string_to_variable_name
...
class AssertionBuilderPresenter:
    ...
    def __init__(self, parent_view):
        ...
        self.assertion_list_presenter = AssertionListPresenter(
            self,
            self.parent_view.tbl_assertions,
            self.parent_view
        )
    ...
    def on_request_body_selected(self, json_path, current_value):
        ...
        item = QTreeWidgetItem([
            ...
            string_to_variable_name(self.current.title, AssertionDataSource.REQUEST_BODY.value, selector),
            selector,
            str(current_value)
        ])
    ...
    def on_response_body_selected(self, json_path, current_value):
        ...
        item = QTreeWidgetItem([
            ...
            string_to_variable_name(self.current.title, AssertionDataSource.RESPONSE_BODY.value, selector),
            selector,
            str(current_value)
        ])
        item.setData(3, ASSERTION_TYPE_ROLE, data_type(current_value))
    ...
    def on_request_header_selection(self, selected_item):
        item = QTreeWidgetItem([
            ...
            string_to_variable_name(self.current.title, AssertionDataSource.REQUEST_HEADER.value,
    ...
    def on_response_header_selection(self, selected_item):
        item = QTreeWidgetItem([
            ...
            string_to_variable_name(self.current.title, AssertionDataSource.RESPONSE_HEADER.value,
                                    selected_item.text(0)),
            selected_item.text(0),
            selected_item.text(1)
        ])
