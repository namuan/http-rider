...
def test_export_single_open_api_v3_document():
    ...
    assert openapi.get("info").get("description") != ""
    assert openapi.get("info").get("title") != ""
    ...
    qp_a = next(
        (p for p in openapi_path.get("get").get("parameters") if p.get("in") == "query" and p.get("name") == "a"),
        None,
    )
